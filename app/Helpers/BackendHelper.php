...
if (!function_exists('check_level')) {
    function check_level() {
        $checkDokter = \App\Models\Dokter::where('id_user', Auth::user()->id)->exists();
        if($checkDokter) {
            return redirect()->intended('dokter');
        } else {
            $checkPerawat = \App\Models\Perawat::where('id_user', Auth::user()->id)->exists();
            if($checkPerawat) {
...
if (!function_exists('check_nakes')) {
    function check_nakes() {
        $checkDokter = \App\Models\Dokter::where('id_user', Auth::user()->id)->exists();
        if($checkDokter) {
            return site_url('dokter', '/');
        } else {
            $checkPerawat = \App\Models\Perawat::where('id_user', Auth::user()->id)->exists();
            if($checkPerawat) {
                return site_url('perawat', '/');
            } else {
                return site_url('apoteker', '/');
            }
        }
    }
}
...
if (!function_exists('site_url')) {
    function site_url($type, $url)
    {
        ...
        {
            case 'perawat':
                return url('perawat/' . $url);
            break;

            case 'dokter':
                return url('dokter/' . $url);
            break;

            case 'apoteker':
                return url('apoteker/' . $url);
            break;
        }
    }
}
