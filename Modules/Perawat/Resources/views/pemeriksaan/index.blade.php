...
@push('scripts')
    ...
    $(function () {
                        ...
                        if(row.is_status_antrian == 1) {
                            return `
                                <div class="d-flex">
                                    ...
                                    <a href="{{ site_url('perawat', 'pemeriksaan/detail') }}/${row.id}" class="btn btn-dark btn-sm ml-2"><i class="fa fa-eye"></i> Detail</a>
                                </div>
                            `;
                        } else if (row.is_status_antrian == 2) {
                            return `
                                <div class="d-flex">
        ...
        $(document).on('click', '.periksa', function () {
            ...
            $.ajax({
                url: `{{ site_url('perawat', 'pemeriksaan/formPeriksa') }}`,
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    id: id_periksa
        ...
        $(document).on('click', '.foto', function () {
            ...
            var id_periksa = $(this).data('id');
            $.ajax({
                url: `{{ site_url('perawat', 'pemeriksaan/formLampiran') }}`,
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}",
                    id: id_periksa
