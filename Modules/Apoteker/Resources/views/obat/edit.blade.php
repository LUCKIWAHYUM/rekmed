        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex">
                    <h6 class="m-0 font-weight-bold text-primary">Perbarui Obat</h6>
                    <p class="m-0 ml-auto small"><span class="text-danger">*</span> Wajib diisi</p>
                </div>
                <div class="card-body">
                    <form action="{{ site_url('dokter', 'obat/update') . '/' . $obat->id }}" method="POST">
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-6 form-group mb-3">
                            ...
                            <input type="text" class="form-control form-control-user {{ $errors->has('name') ? 'is-invalid' : '' }}" id="email" name="name" aria-describedby="emailHelp" placeholder="Enter Nama Obat." value="{{ $obat->name }}">
                        ...
                        <div class="col-md-6 form-group mb-3">
                            ...
                            <input type="text" class="form-control form-control-user {{ $errors->has('kode_obat') ? 'is-invalid' : '' }}" id="email" name="kode_obat" aria-describedby="emailHelp" placeholder="Enter Kode Obat." value="{{ $obat->kode_obat }}">
                        ...
                        <div class="col-md-12 form-group mb-3">
                            ...
                            <textarea class="form-control form-control-user {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="exampleFormControlTextarea1" rows="3" placeholder="Enter Keterangan.">{{ $obat->description }}</textarea>
                    ...
                    <div class="row">
                        <div class="col-md-4 form-group mb-3">
                            ...
                            <select name="category" class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}">
                                ...
                                @foreach([1, 2, 3, 4, 5, 6] as $category)
                                    ...
                                    <option value="{{ $category }}" {{ $obat->category == $category ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 form-group mb-3">
                            ...
                            <input type="number" class="form-control form-control-user {{ $errors->has('stock') ? 'is-invalid' : '' }}" id="email" name="stock" aria-describedby="emailHelp" placeholder="Enter Stok." value="{{ $obat->stock }}">
                            @if($errors->has('stock'))<span class="small text-danger mt-2">{{ $errors->first('stock') }}</span>@endif
                        </div>
                        <div class="col-md-4 form-group mb-3">
                            <label for="exampleEmail1">Status<span class="text-danger">*</span></label>
                            <select name="status" class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}">
                                <option value="">Pilih Status</option>
                                @foreach([1, 2] as $status)
                                    @php $name = $status == 1 ? 'Aktif' : 'Tidak Aktif' @endphp
                                    <option value="{{ $status }}" {{ $obat->status == $status ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-12 form-group mb-3">
                            ...
                            <input type="number" class="form-control form-control-user {{ $errors->has('price') ? 'is-invalid' : '' }}" id="email" name="price" aria-describedby="emailHelp" placeholder="Enter Harga." value="{{ $obat->price }}">
                            @if($errors->has('price'))<span class="small text-danger mt-2">{{ $errors->first('price') }}</span>@endif
                        </div>
                    </div>
                    <div class="form-group mb-0">
                        <button type="submit" class="btn btn-primary">Simpan</button>
                        <a href="{{ site_url('apoteker', 'obat') }}" class="btn btn-light btn-light">Kembali</a>
                    </div>
                    {{ Form::close() }}
                </div>
            </div>
