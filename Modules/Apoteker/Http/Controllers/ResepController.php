...
class ResepController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('status_obat', function($row){
                    $checkObat = Resep::where('no_periksa', $row->no_periksa)->first();
                    if($checkObat) {
                        if($checkObat->status == 1) {
                            return '<span class="mb-1 badge font-medium badge-success py-2 px-3 fs-7">Sudah Diambil</span>';
                        } else {
                            return '<span class="mb-1 badge font-medium badge-danger py-2 px-3 fs-7">Belum Diambil</span>';
                        }
                    } else {
                        return '-';
                    }
                })
                ->addColumn('is_status_obat', function($row){
                    $checkObat = Resep::where('no_periksa', $row->no_periksa)->first();
                    if($checkObat) {
                        return $checkObat->status;
                    } else {
                        return '-';
                    }
                })
                ...
                ->rawColumns(['nama', 'is_resep_exist', 'is_status_obat', 'status_obat', 'no_rm', 'poli', 'is_status', 'register', 'diagnosa', 'tindakan', 'tanggal_kunjungan'])
                ->make(true);
        }

        $data = [
            'subtitle' => 'Resep Obat Pasien'
        ];

        return view('apoteker::resep-obat.index', compact('data'));
    }
    ...
    public function detail($id)
    {
        ...
        if($detail) {
            ...
            return view('apoteker::resep-obat.detail', compact('data', 'detail', 'resep', 'pemeriksaan'));
        } else {
    ...
    public function print($id)
    {
        ...
        if($detail) {
            ...
            $pdf->SetFont('Arial', 'B', 12);
            $pdf->Cell(90, 10, "Status", 1, 0, 'C');
            $pdf->SetFont('Arial', '', 12);
            $pdf->Cell(100, 10, $resep->status == 1 ? 'Telah diambil di Rumah Sakit / Dibeli diluar Rumah Sakit' : 'Belum diambil', 1, 0, 'C');
            $pdf->Ln();
    ...
    public function done($id)
    {
        ...
        if($resep) {
            $resep->status = 1;
            foreach(explode(',', $resep->id_obat) as $obat) {
                $obat = \App\Models\Obat::where('id', $obat)->first();
                $obat->stock = $obat->stock - 1;
                $obat->save();
            }
            if($resep->save()) {
                return redirect()->back()->with('success', 'Resep obat pasien dinyatakan telah diterima, dan pasien dapat mengambil obat diapotik jika pasien memilih untuk mengambil obat');
            } else {
